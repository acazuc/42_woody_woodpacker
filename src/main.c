...
int main(int ac, char **av)
{
	...
	*env.start_addr = env.new_sec_hdr.sh_addr;
	//env.new_sec_data = "\xbf\x01\x00\x00\x00\x48\x8d\x35\x0c\x00\x00\x00\xba\x0f\x00\x00\x00\x48\x89\xf8\x0f\x05\xc3\xc3\x2e\x2e\x2e\x2e\x57\x4f\x4f\x44\x59\x2e\x2e\x2e\x2e\x0a\x00\x00\x00\x00\x00\x00\x00\x00\x00";
	//env.new_sec_hdr.sh_size = 48;
	*env.sect_off += env.new_sec_hdr.sh_size;
	...
	if (wreight(fd, env.bin, env.new_sec_hdr.sh_offset) != (int64_t)env.new_sec_hdr.sh_offset)
	{
		perror("woody_woodpacker: Failed to write to woody 1");
		exit(EXIT_FAILURE);
	}
	if (wreight(fd, env.new_sec_data, env.new_sec_hdr.sh_size) != (int64_t)env.new_sec_hdr.sh_size)
	{
		perror("woody_woodpacker: Failed to write new sect");
		exit(EXIT_FAILURE);
	}
	printf("pos: %lu\n", env.new_sec_hdr.sh_offset);
	if (wreight(fd, env.bin + env.new_sec_hdr.sh_offset, env.new_sec_hdr_pos - env.new_sec_hdr.sh_offset) != (int64_t)(env.new_sec_hdr_pos - env.new_sec_hdr.sh_offset))
	{
		perror("woody_woodpacker: Failed to write to woody 2");
		exit(EXIT_FAILURE);
	}
	if (wreight(fd, &env.new_sec_hdr, sizeof(env.new_sec_hdr)) != sizeof(env.new_sec_hdr))
	{
		perror("woody_woodpacker: Failed to write new sect hdr");
		exit(EXIT_FAILURE);
	}
	if (wreight(fd, env.bin + env.new_sec_hdr_pos, env.bin_len - env.new_sec_hdr_pos) != (int64_t)(env.bin_len - env.new_sec_hdr_pos))
	{
